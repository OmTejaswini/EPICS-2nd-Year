...
app.post('/', (req,res) => {
    ...
    connection.query('SELECT * from bookings where BookingDate = ? and (StartTime between ? and ? or EndTime between ? and ?)', [dt, st, et, st, et], (err,results,fields) => {
        ...
        if(results.length == 0){
            res.render('booked', {title:'Allocted room',sampleData:r})
            connection.query('INSERT INTO bookings (RName , BookingDate, StartTime, EndTime) VALUES (?, ?, ?, ?)',[r, dt, st, et,1],)
        }
        else{
                let g = new Graph(20);
    ...
    function func(){
        ...
        connection.query('SELECT * from rooms', (err,results,fields) => {
            ...
            connection.query('SELECT * from bookings where BookingDate = ? and (StartTime between ? and ? or EndTime between ? and ?)', [dt, st, et, st, et], (err,results,fields) => {
                ...
                if(results.length == 0){
                    res.render('booked.ejs', {title:'Allocted room',sampleData:r})
